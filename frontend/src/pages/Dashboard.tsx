...
export const Dashboard: React.FC<DashboardProps> = ({ setActivePage }) => {
  ...
  // RENDER FINANCIAL INSTITUTION DASHBOARD
  if (currentUser.role === 'FINANCIAL_INSTITUTION') {
    ...
    // Risk Categories formatting for Recharts
      ...
      <div className="space-y-6 max-w-6xl mx-auto animate-fadeIn pb-8 text-white">
        ...
        <GlassPanel className="p-6 relative overflow-hidden flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          ...
          <div className="flex space-x-3">
            {setActivePage && (
              <>
                <GlassButton variant="primary" onClick={() => setActivePage('credit')} className="px-4 py-2 font-bold text-xs uppercase bg-emerald-600 hover:bg-emerald-500 border-none">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <GlassPanel className="p-6 space-y-4 flex flex-col justify-between">
            ...
            <p className="text-[10px] text-[var(--mf-text-secondary)] leading-relaxed italic border-t border-white/5 pt-3">
              Indicative capacity is not a loan approval. Final lending decisions require institutional underwriting.
            </p>
          </GlassPanel>
        </div>
  ...
  // RENDER ADMIN DASHBOARD (SYSTEM OPERATIONS CENTER)
